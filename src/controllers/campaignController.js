...
export const createCampaignWithTemplate = async (req, res) => {
  try {
    ...
    if (isNaN(template_id)) {
      return res.status(400).json({ error: 'Invalid template_id. It should be a number.' });
    }
    ...
    res.status(201).json({ message: 'Campaign created with template', campaignId });
  } catch (error) {
    ...
    if (error.response) {
      ...
      res.status(status).json({ error: data.detail || 'Failed to create campaign with template' });
    } else {
      // Errores generales
      res.status(500).json({ error: 'Failed to create campaign with template' });
    }
  }
};
